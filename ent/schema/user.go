...
func (User) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("username").MinLen(3).MaxLen(30).Unique().Nillable().Optional().Comment("username of a user"),
		...
		field.Time("last_seen").Nillable().Optional().Comment("the time of user's last seen (was online)"),
